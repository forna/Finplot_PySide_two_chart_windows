...
class FinplotWidget(QWidget):
    ...
    def load_df(self, symbol, df):
        ...
        if len(df.index) != 0:  # Ensure there is data in the dataframe
            ...
            cadlesticks = fplt.candlestick_ochl(df[df_cols_list])
            cadlesticks.colors.update(dict(bull_body="#00AA00", bull_shadow="#fff", bull_frame="#00AA00",
                                           bear_body="#D20000", bear_shadow="#fff", bear_frame="#D20000"))
            cadlesticks.x_offset = 0.5
            fplt.refresh()
...
class MainWindow(QMainWindow):
    def __init__(self, parent, df_dict):
        ...
        # Stock Chart 2
        combo_stock_chart_widget2 = instantiate_combo_chart_widget(df_dict)
